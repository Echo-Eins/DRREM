...
Энергия такта (для работающих уровней; x̄ = Σ_m c_m ⊙ trace_m и a постоянны внутри такта):
  E(s) = Σ Φ(s_i) − ½ sᵀ S s − sᵀ (W x̄ + I − g ⊙ a) − (g_d/β_d) Σ_ℓ logsumexp(β_d Ξ_ℓ s_ℓ).
...
class MachineV2Config:
    N: int = 256
    L: int = 1
    rho: Literal["hardsig", "relu", "sigmoid"] = "hardsig"  # sigmoid — гладкая, для диагностики линеаризации
    theta: float = 0.0
    alpha: float = 0.5
    gamma_in: float = 0.25
    g_S: float = 0.4
...
class MachineV2:
    def __init__(self, cfg: MachineV2Config, device="cpu"):
        ...
        for hs in cfg.horizons:
            self.E_r.append((torch.randn(len(hs), 256, N, generator=gen) * cfg.g_r / math.sqrt(N)).to(self.device))
    ...
    def instrumented(self):
        ...
        saved = (self.E_r, self.Xi, self.c, self.g_adapt, self.kappa)
        ...
        try:
            yield W_leaf, self.E_r, self.Xi, self.c, self.g_adapt, self.kappa
        finally:
            self.E_r, self.Xi, self.c, self.g_adapt, self.kappa = saved
    ...
    def dam_drive(self, s: torch.Tensor, Xi: list[torch.Tensor] | None = None) -> torch.Tensor | None:
        ...
        s_l = s.view(B, L, N)
        out = [torch.softmax(self.cfg.dam_beta * (s_l[:, l] @ Xi[l].T), dim=-1) @ Xi[l] for l in range(L)]
        return self.cfg.dam_gain * torch.stack(out, 1).reshape(B, self.cfg.D)
    ...
    def dam_jacobian_apply(self, v: torch.Tensor, s: torch.Tensor, Xi=None) -> torch.Tensor:
        ...
        return self.cfg.dam_gain * self.cfg.dam_beta * torch.stack(out, 1).reshape(B, self.cfg.D)
    ...
    def fixed_point_residual(self, x, I, xbar=None, bias=None, unit_mask=None) -> torch.Tensor:
        """|x − drive| / |x| по активным единицам (замороженные уровни не обязаны быть в равновесии)."""
    ...
    def h1_error_force(self, s: torch.Tensor, next_byte: torch.Tensor) -> torch.Tensor:
        ...
        f = ((yoh - p) @ self.E_r[0][0]) / self.cfg.tau_r
    ...
    def logits(self, s: torch.Tensor, l: int) -> torch.Tensor:
        B, N = s.shape[0], self.cfg.N
        return torch.einsum("bn,hvn->bhv", s.view(B, self.cfg.L, N)[:, l], self.E_r[l]) / self.cfg.tau_r
    ...
    def nudge_force(self, s, Y, V) -> torch.Tensor:
        ...
        for l in range(self.cfg.L):
            ...
            out[:, l] = torch.einsum("bhv,hvn->bn", err, self.E_r[l]) * (self.level_w[l] / self.cfg.tau_r)
    ...
    def energy(self, x, I, xbar=None, bias=None) -> torch.Tensor:
        ...
        s = self.rho(x)
        W = self.W()
        E = -0.5 * ((s @ W.T) * s).sum(-1) - (s * I).sum(-1) + (0.5 * s * s + self.theta * s).sum(-1)
        if xbar is not None:
        ...
        if self.Xi:
            ...
            s_l = s.view(B, L, N)
            for l in range(L):
                E = E - (self.cfg.dam_gain / self.cfg.dam_beta) * torch.logsumexp(self.cfg.dam_beta * (s_l[:, l] @ self.Xi[l].T), dim=-1)
        return E
    ...
    def update_slow(self, state: State, s_free: torch.Tensor, unit_mask: torch.Tensor) -> None:
        ...
        if state.adapt is not None:
            ...
            new = dec_a * state.adapt + (1.0 - dec_a) * s_free
    ...
    def state_dict(self) -> dict:
        d = {"S": self.S, "A": self.A, "E_in": self.E_in, "E_r": self.E_r, "theta": self.theta, "Xi": self.Xi,
    ...
    def to_dtype(self, dtype: torch.dtype) -> "MachineV2":
        for name in ("S", "A", "E_in", "theta", "mask", "level_w", "tick_thr"):
