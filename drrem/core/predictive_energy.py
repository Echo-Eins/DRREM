...
class PredictiveEnergy:
    def energy_context(self, st, drive, *, differentiate_drive=False):
        cfg = self.cfg
        ...
        context = st.msg + drive
        if not differentiate_drive:
    ...
    def energy_tick(self, st, drive, learn=False, hops=None, *, differentiate_drive=False):
        cfg = self.cfg
        H = cfg.hops if hops is None else hops
        if H < 1:
            raise ValueError('hops must be positive')
        ctx = self.energy_context(st, drive, differentiate_drive=differentiate_drive)
