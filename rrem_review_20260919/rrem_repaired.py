...
class RREM:
    ...
    def __init__(self,cfg: Cfg):
        ...
        self.E=rnd(cfg.H_pred,256,N)*cfg.g_r/math.sqrt(N)
        self.E_bias=torch.zeros(cfg.H_pred,256,device=self.dev,dtype=self.dtype)
        self.E_in=self.E[0].clone()  # active only if tie_input=False
    ...
    def emitter(self,u: torch.Tensor,theta: torch.Tensor,hop: int):
        ...
        return msg,{'u':u,'theta':theta,'content':content,'z':z,'q':q,'den':den,'route':route,'psi':psi,'hop':hop}
    ...
    def emit_vjp(self,cache: dict,signal: torch.Tensor):
        ...
        if cfg.use_phase:
            ang=2*math.pi*cache['hop']/cfg.phase_period-self.phi
    ...
    def tick(self,st: State,I: torch.Tensor,learn: bool=False,*,hops: int|None=None):
        ...
        elig=torch.zeros(st.u.shape[0],cfg.M,cfg.D,device=self.dev,dtype=self.dtype)
        ...
        for k in range(H):
            ...
            field=pre@W[0].T+slow+I
            if cfg.neuron_mode=='legacy':field=field-st.a
    ...
    def logits(self,msg: torch.Tensor,level: int):
        ...
        return torch.einsum('bn,hvn->bhv',z,self.E)/self.cfg.tau_r+self.E_bias
    ...
    def _input_update(self,dI: torch.Tensor,byte: torch.Tensor):
        ...
        if self.cfg.tie_input:self.grad['E'][0].index_add_(0,byte,dr)
    ...
    def learn_tick(self,st: State,out: dict,byte: torch.Tensor,Y: torch.Tensor,V: torch.Tensor,
        ...
        for msg,cache in zip(out['msgs'],out['records']):
            ...
            for l in range(cfg.L):
                ...
                self.grad['E']+=torch.einsum('bhv,bn->hvn',err,z)/cfg.tau_r
                self.grad['E_bias']+=err.sum(0)
                signal[:,l*cfg.N:(l+1)*cfg.N]+=torch.einsum('bhv,hvn->bn',err,self.E)/cfg.tau_r
                ce=-(lp.gather(-1,Y[:,:,None]).squeeze(-1)*valid)
                loss_sum+=float(ce.sum())*pred_weight/den
                ce_sample+=ce.sum(-1)/valid.sum(-1).clamp_min(1)/cfg.L
    ...
    def apply_batch(self):
        ...
        for name in self.param_names:
            ...
            p=getattr(self,name);p.add_(d,alpha=self.zeta*scale)
...
def evaluate(mach: RREM,batches: Iterable,*,hops: int|None=None):
    ...
    return {'bpb':bpb.cpu().tolist(),'bpb_h1':float(bpb[0,0]),'bpb_mean_all_h':float(bpb[0].mean()),
