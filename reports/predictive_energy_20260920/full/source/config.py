...
class DataConfig:
    ...
    test_docs: int = 0  # separately reserved; never used by development evaluation
...
class LearnConfig:
    ...
    adam_groups: tuple[str, ...] = ("S", "A", "E", "Xi", "c", "g", "k", "Ein", "gd")  # какие группы вести Adam'ом (остальные — SGD)
    ...
    lr_gd: float = 0.01  # шаг усиления плотной памяти (скаляр на уровень)
    c_profile: bool = False  # c — профиль: после шага Σ_m |c_im| возвращается к начальной сумме (перераспределение, не усиление)
    # доверительный шаг по группе (LARS-образно): ‖Δp‖/max(‖p‖,‖p₀‖) приводится К цели, а не только обрезается
    # сверху. Лечит обусловленность по масштабам групп (измерено на 512×3: S 1,7e-3, чтение 1,1e-3,
    # прототипы 3,0e-5 / 1,9e-6 / 1,9e-7 за обновление) и не трогает ни одну координату по отдельности —
    # в отличие от Adam, который раздувает мелкие шумные координаты локального сигнала (P1 §12).
    # Осторожно: при слабом или несогласованном сигнале фиксированный относительный шаг — случайное
    # блуждание (P1 §16 про c/g/κ), поэтому свойства нейрона в trust_groups по умолчанию не входят.
    step_mode: Literal["clip", "trust"] = "clip"
    trust_ratio: float = 1e-3  # целевое ‖Δp‖/‖p‖ за обновление для групп из trust_groups
    trust_groups: tuple[str, ...] = ("S", "A", "E", "Xi")
    # предел усиления: доверительный шаг поднимает скорость группы к цели, но не более чем в
    # trust_max_gain раз (0 — без предела). Иначе группа со слабым сигналом получает ×1000 и
    # превращается в случайное блуждание — ровно то, что §16 отчёта нашёл у свойств нейрона.
    trust_max_gain: float = 0.0
