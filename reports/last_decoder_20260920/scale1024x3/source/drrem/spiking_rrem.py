...
from drrem.core.event_stdp import PairSTDP,STDPConfig,IntegratedPairSTDP,response_credit_schedule
...
class SpikeConfig:
    ...
    readout:str='last'  # legacy_all is only for reproducing old experiments
    mtp_weight:float=1.  # CE(next byte) + weight * mean(CE(h2..h8))
    teacher_transport:str='recurrent'  # desired events travel through the real W
    integration:str='explicit'  # exact batch-integrated STDP can avoid B*D*D storage
    seed:int=20260923
    ...
    def validate(self):
        ...
        if self.readout not in ('last','legacy_all'):raise ValueError('unknown readout objective')
        if self.teacher_transport not in ('free','recurrent','transpose'):raise ValueError('unknown teacher transport')
        if self.integration not in ('explicit','integrated'):raise ValueError('unknown STDP integration')
        if self.integration=='integrated' and (self.learning!='resume' or self.readout!='last'):raise ValueError('integrated eligibility requires resume and last readout')
        if self.mtp_weight<0:raise ValueError('negative MTP weight')
        if self.readout=='last' and self.learning!='resume':raise ValueError('last-only training requires transported teacher credit')
...
class SpikingRREM:
    ...
    def __init__(self,cfg:SpikeConfig):
        ...
        self.updates=0;self.seen_targets=0;self.seen_response_bytes=0

    def input_weights(self):
        e=self.E[0] if self.cfg.tie_input else self.E_in
        return self.cfg.input_gain*e/e.norm(dim=-1,keepdim=True).clamp_min(1e-8)

    def init_state(self,batch,learning=False,coefficients=None):
        c=self.cfg;D=c.D
        zeros=lambda *shape:torch.zeros(*shape,device=self.dev,dtype=self.dtype)
        factory=PairSTDP if coefficients is None else IntegratedPairSTDP
        kw={} if coefficients is None else {'coefficients':coefficients}
        pair=factory(batch,len(c.delays),D,D,cfg=c.stdp,device=self.dev,dtype=self.dtype,**kw) if learning and not c.freeze_core else None
        inp=factory(batch,1,c.N,256,cfg=c.stdp,device=self.dev,dtype=self.dtype,**kw) if learning and c.learn_input else None
        ...
        if learning and c.learning=='resume' and (pair is not None or inp is not None):
            ...
            # STDP presynaptic arrivals ALWAYS come from the free network.
            # The desired compartment can propagate its own spikes through W,
            # carrying last-decoder supervision to other levels without heads.
            state.teacher={'u':state.u.clone(),'adaptation':state.adaptation.clone(),
                'refractory':state.refractory.clone(),'differences':zeros(batch,D)}
            if c.teacher_transport!='free':state.teacher['history']=state.history.clone()
        return state

    def recurrent_field(self,arrivals,weights,transpose=False):
        if self.cfg.readout=='legacy_all':
            return torch.einsum('bmi,mij->bj' if transpose else 'bmi,mji->bj',arrivals,weights)
        # Avoid einsum's materialization of a permuted M*D*D weight tensor on
        # every microtick. Each axonal-delay matrix remains full and dense.
        matrix=weights if transpose else weights.transpose(1,2)
        return torch.bmm(arrivals.transpose(0,1),matrix).sum(0)
    ...
    def tick(self,state,byte,active,*,weights=None,input_weights=None,record=False,teacher_signal=None):
        ...
        for hop in range(c.hops):
            ...
            field=self.recurrent_field(arrivals,W)
            ...
            if state.teacher is not None:
                ...
                teacher_field=field
                if c.teacher_transport!='free':
                    teacher_arrivals=target_state['history'][:,self.delay_index]
                    if c.teacher_transport=='transpose':
                        # Transport deviations along transpose synapses: a neuron
                        # receives credit through its outgoing forward weights.
                        # This is a teaching path, NOT an exact spike derivative.
                        teacher_field=field+self.recurrent_field(teacher_arrivals-arrivals,W,transpose=True)
                    else:
                        teacher_field=self.recurrent_field(teacher_arrivals,W)
                        if hop==0:teacher_field[:,:c.N]+=table[byte]
                tu=self.leak*target_state['u']+(1-self.leak)*self.tonic+teacher_field+c.teacher_gain*feedback
                ...
                if c.teacher_transport!='free':
                    thistory=torch.cat((ts[:,None],target_state['history'][:,:-1]),1)
                    target_state['history'].copy_(torch.where(v[:,None],thistory,target_state['history']))
                ...
                target_state['differences'].add_(plastic_post.abs())
            if state.stdp is not None:
                state.stdp.arrive_and_fire(arrivals,plastic_post,active)
                if c.rule=='no_eligibility':
                    state.stdp.clear_eligibility() if isinstance(state.stdp,IntegratedPairSTDP) else state.stdp.eligibility.zero_()
            if state.input_stdp is not None:
                state.input_stdp.arrive_and_fire(input_spikes[:,None],plastic_post[:,:c.N],active)
                if c.rule=='no_eligibility':
                    state.input_stdp.clear_eligibility() if isinstance(state.input_stdp,IntegratedPairSTDP) else state.input_stdp.eligibility.zero_()
    ...
    def readout_levels(self):
        return (self.cfg.L-1,) if self.cfg.readout=='last' else tuple(range(self.cfg.L))

    def horizon_weights(self,valid):
        if self.cfg.readout=='legacy_all':
            return valid.to(self.dtype)/valid.sum(-1,keepdim=True).clamp_min(1)
        w=valid.to(self.dtype).clone()
        if self.cfg.horizons>1:w[:,1:]*=self.cfg.mtp_weight/(self.cfg.horizons-1)
        return w
    ...
    def feedback(self,features,target,valid):
        ...
        """
        c=self.cfg
        weights=self.horizon_weights(valid)
        m=torch.zeros_like(features)
        onehot=F.one_hot(target,256).to(self.dtype)
        for l in self.readout_levels():
            error=(onehot-self.logits(features,l).softmax(-1))*weights[:,:,None]/len(self.readout_levels())
    ...
    def teaching_signal(self,features,target,valid):
        ...
        for l in self.readout_levels():
            ...
            error=(F.one_hot(target,256).to(self.dtype)-lp.exp())*weights[:,:,None]/len(self.readout_levels())
            modulator[:,sl]=c.readout_gain*torch.einsum('bhv,hvn->bn',error,self.E)
            dE+=torch.einsum('bhv,bn->hvn',error,features[:,sl])/n
            db+=error.sum(0)/n
            loss+=float((-(lp.gather(-1,target[:,:,None]).squeeze(-1)*weights).sum(-1))[active].mean())/len(self.readout_levels())
    ...
    def train_batch(self,batch):
        c=self.cfg;b=batch.to(self.dev)
        integrated=c.integration=='integrated'
        coefficients=response_credit_schedule(b.active[:,:-1],b.loss_mask[:,:-1]&b.active[:,:-1],c.hops,c.stdp.tau_eligibility,self.dtype) if integrated else None
        state=self.init_state(len(b.x),learning=True,coefficients=coefficients)
        ...
        for t in range(b.T-1):
            ...
            supervised=v.any(-1)
            mass=int(supervised.sum()) if c.readout=='last' else 1
            accum['E']+=mass*dE;accum['E_bias']+=mass*db
            head_signal+=mass*dE[0]
            if state.stdp is not None and not integrated:
                ...
                accum['S']+=mass*project(direction,1,self.mask)
                accum['A']+=mass*project(direction,-1,self.mask)
            if state.input_stdp is not None and not integrated:
                if c.learning=='resume':
                ...
                if c.tie_input:
                    accum['E'][0]+=mass*c.input_credit*inp
                    input_signal+=mass*c.input_credit*inp
                else:accum['E_in']+=mass*inp
            ticks+=mass;losses.append(mass*loss);self.seen_targets+=int(v.sum())
            self.seen_response_bytes+=int(supervised.sum())
        if not ticks:return {'no_update':True}
        if integrated:
            sign=-1. if c.rule=='reverse' else 1.
            if state.stdp is not None:
                direction=sign*state.stdp.result()/c.teacher_gain
                accum['S']+=project(direction,1,self.mask);accum['A']+=project(direction,-1,self.mask)
            if state.input_stdp is not None:
                inp=self.input_pullback(sign*state.input_stdp.result()[0]/c.teacher_gain)
                if c.tie_input:
                    accum['E'][0]+=c.input_credit*inp;input_signal+=c.input_credit*inp
                else:accum['E_in']+=inp
        stats={'loss_nats':sum(losses)/ticks,'spike_rate_by_level':(activity/max(activity_ticks,1)).view(c.L,c.N).mean(-1).tolist(),
               ...
               'eligibility_norm':float((state.stdp.total if integrated else state.stdp.eligibility).norm()) if state.stdp else 0.,
               'eligibility_statistic':'integrated_sum' if integrated else 'final_trace',
               'teacher_difference_rate_by_level':(state.teacher['differences'].sum(0)/max(activity_ticks,1)).view(c.L,c.N).mean(-1).tolist() if state.teacher else [],
    ...
    @torch.no_grad()
    def checkpoint(self):
        return {'version':2,'dynamics':'event-stdp-rrem-20260923','config':asdict(self.cfg),
                ...
                'updates':self.updates,'seen_targets':self.seen_targets,'seen_response_bytes':self.seen_response_bytes}

    @classmethod
    def from_checkpoint(cls,ck,device=None):
        if ck.get('dynamics')!='event-stdp-rrem-20260923' or ck.get('version') not in (1,2):raise ValueError('incompatible spiking checkpoint')
        cfg=dict(ck['config'])
        if ck['version']==1:
            cfg.update(readout='legacy_all',teacher_transport='free',integration='explicit')
        cfg['stdp']=STDPConfig(**cfg['stdp'])
        ...
        m.updates=ck['updates'];m.seen_targets=ck['seen_targets'];m.seen_response_bytes=ck.get('seen_response_bytes',0)
        return m
...
def evaluate_spiking(m,batches,*,per_document=False,reset_history=False):
    c=m.cfg
    levels=m.readout_levels();K=len(levels)
    totals=torch.zeros(K,c.horizons,device=m.dev,dtype=torch.float64);counts=torch.zeros(c.horizons,device=m.dev,dtype=torch.float64)
    ...
    for batch in batches:
        ...
        dc=torch.zeros(len(b.x),K,c.horizons,device=m.dev,dtype=torch.float64)
        ...
        for t in range(b.T-1):
            ...
            for k,l in enumerate(levels):
                ...
                dc[:,k]+=(-lp.gather(-1,y[:,:,None]).squeeze(-1)*v).double()
    ...
    return {'readout_levels':list(levels),'bpb':bpb.tolist(),'bpb_h1':float(bpb[-1,0]),'bpb_mean_all_h':float(bpb[-1].mean()),
