...
class OpenOrcaBytes:
    def __init__(self, cfg: DataConfig, root: Path | None = None):
        ...
        n = len(self.prompts)
        if min(cfg.heldout_docs, cfg.test_docs) < 0 or cfg.heldout_docs + cfg.test_docs >= n:
            raise ValueError('development/test sizes must leave training documents')
        perm = np.random.default_rng(cfg.split_seed).permutation(n)
        self.heldout_ids = np.sort(perm[: cfg.heldout_docs])
        self.test_ids = np.sort(perm[cfg.heldout_docs : cfg.heldout_docs + cfg.test_docs])
        self.train_ids = np.sort(perm[cfg.heldout_docs + cfg.test_docs :])
        ...
        self.test_ids = self.test_ids[[len(self.responses[i]) > 0 for i in self.test_ids]]
    ...
    def heldout_batches(self, n_batches: int, batch: int | None = None, seed: int = 0):
        ...
        return [self.make_batch(order[i:i+B]) for i in range(0, min(len(order), n_batches*B), B)]

    def test_batches(self, batch: int | None = None):
        B = batch or self.cfg.batch
        return [self.make_batch(self.test_ids[i:i+B]) for i in range(0, len(self.test_ids), B)]
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "test_docs": int(len(self.test_ids)),
