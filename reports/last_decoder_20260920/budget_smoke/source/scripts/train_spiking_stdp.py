...
def main():
    ...
    p.add_argument('--length-bucket',type=int,default=0,help='response-length sorting window, multiple of batch, only with byte budget')
    ...
    p.add_argument('--learning',choices=['resume'],default='resume')
    ...
    budget=select_response_budget(data,a.response_bytes,a.seed+3,bucket=a.length_bucket,batch=a.batch) if a.response_bytes is not None else None
    ...
    if not a.resume:
        emit({'step':0,'dev':evaluate_spiking(m,dev),'unigram':unigram_score(dev,prior,cfg.horizons)})
        save()
    stopping=[]
