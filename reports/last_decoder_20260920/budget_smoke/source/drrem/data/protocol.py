...
def data_protocol(data, batch, sampler_seed, dev_batches, bias_init):
    ...
    for name, ids in split.items():
        for i in ids:
            key = hashlib.sha256(data.prompts[i]+b'\0'+getattr(data,'original_responses',data.responses)[i]).digest()
...
def restore_openorca_protocol(protocol):
    """Restore explicit document IDs; never infer split seed from model seed."""
    import numpy as np
    from drrem.config import DataConfig
    from drrem.data.openorca import OpenOrcaBytes
    source=protocol['files']['parquet']
    if file_digest(source['path'])!=source['sha256']:raise ValueError('dataset bytes changed')
    split=protocol['partitions']
    cfg=DataConfig(path=source['path'],prompt_max=protocol['prompt_max'],resp_max=protocol['resp_max'],
        batch=protocol['batch'],heldout_docs=len(split['dev']),test_docs=len(split['test']))
    data=OpenOrcaBytes(cfg)
    if data.source_row.tolist()!=protocol['source_ids']:raise ValueError('source document identity changed')
    data.train_ids=np.asarray(split['train']);data.heldout_ids=np.asarray(split['dev']);data.test_ids=np.asarray(split['test'])
    for i,cap in protocol.get('response_budget',{}).get('response_caps',{}).items():
        data.responses[int(i)]=data.responses[int(i)][:cap]
    return data
