        ...
        for hop in range(c.hops):
            ...
            if state.teacher is not None:
                ...
                if c.teacher_transport!='free':
                    ...
                    if c.teacher_transport=='transpose':
                        ...
                        teacher_field=field+torch.einsum('bmi,mij->bj',teacher_arrivals-arrivals,W)
                    else:
                        teacher_field=torch.einsum('bmi,mji->bj',teacher_arrivals,W)
            ...
            if state.stdp is not None:
                ...
                if c.rule=='no_eligibility':
                    (state.stdp.total if isinstance(state.stdp,IntegratedPairSTDP) else state.stdp.eligibility).zero_()
            if state.input_stdp is not None:
                state.input_stdp.arrive_and_fire(input_spikes[:,None],plastic_post[:,:c.N],active)
                if c.rule=='no_eligibility':
                    (state.input_stdp.total if isinstance(state.input_stdp,IntegratedPairSTDP) else state.input_stdp.eligibility).zero_()
