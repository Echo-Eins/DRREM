"""Event-driven S/A RREM with complete pair-STDP and a semantic third factor.
...
The shared dictionary uses the exact softmax delta rule, and optionally the
STDP signal of the tied input synapses. Optimizer: unmodified torch Adam.
No surrogate spike derivative, contrastive phase, BPTT or transformer block.
"""
...
class SpikeConfig:
    ...
    core_lr:float=.0003
    head_lr:float=.003
    input_credit:float=.1
    tie_input:bool=True
    freeze_core:bool=False
    freeze_head:bool=False
    learn_input:bool=True
    rule:str='stdp'
    learning:str='modulated'  # 'resume': teacher/actual spike-train STDP difference
...
class SpikingRREM:
    ...
    def init_state(self,batch,learning=False):
        ...
        if learning and c.learning=='resume' and (pair is not None or inp is not None):
            ...
            state.teacher={'u':state.u.clone(),'adaptation':state.adaptation.clone(),
                ...
                'stdp':PairSTDP(batch,len(c.delays),D,D,cfg=c.stdp,device=self.dev,dtype=self.dtype) if pair else None,
                'input_stdp':PairSTDP(batch,1,c.N,256,cfg=c.stdp,device=self.dev,dtype=self.dtype) if inp else None}
        return state
    ...
    def tick(self,state,byte,active,*,weights=None,input_weights=None,record=False,teacher_signal=None):
        ...
        for hop in range(c.hops):
            ...
            if state.stdp is not None:
                state.stdp.arrive_and_fire(arrivals,spikes,active)
                if c.rule=='no_eligibility':state.stdp.eligibility.zero_()
            if state.input_stdp is not None:
                state.input_stdp.arrive_and_fire(input_spikes[:,None],spikes[:,:c.N],active)
            if state.teacher is not None:
                ...
                if target_state['stdp'] is not None:target_state['stdp'].arrive_and_fire(arrivals,ts,active)
                if target_state['input_stdp'] is not None:target_state['input_stdp'].arrive_and_fire(input_spikes[:,None],ts[:,:c.N],active)
    ...
    def input_pullback(self,signal):
        e=self.E[0] if self.cfg.tie_input else self.E_in
        norm=e.norm(dim=-1,keepdim=True).clamp_min(1e-8);unit=e/norm
        g=signal.T
        return self.cfg.input_gain*(g-unit*(g*unit).sum(-1,keepdim=True))/norm
    ...
    def train_batch(self,batch):
        ...
        for t in range(b.T-1):
            ...
            teacher=(lambda features:self.teaching_signal(features,y,v)[0]) if t>=b.P-1 and bool(v.any()) and state.teacher is not None else None
            ...
            if state.stdp is not None:
                if c.learning=='resume':
                    ...
                    direction=(state.teacher['stdp'].modulated(ones,supervised)-state.stdp.modulated(ones,supervised))/c.teacher_gain
            ...
            if state.input_stdp is not None:
                if c.learning=='resume':
                    ...
                    inp=(state.teacher['input_stdp'].modulated(ones,supervised)-state.input_stdp.modulated(ones,supervised))[0]/c.teacher_gain
                else:inp=state.input_stdp.modulated(m[:,:c.N],supervised)[0]
                inp=self.input_pullback(inp)
                if c.tie_input:accum['E'][0]+=c.input_credit*inp
                else:accum['E_in']+=inp
