...
class Cfg:
    ...
    def validate(self) -> None:
        ...
        if self.learning_rule=='energy':
            ...
            if self.ff_weight or self.lam_energy or self.reward_weight or self.carry_elig or self.gate_state or self.phase_input or self.oja or self.lam_hop:
                raise ValueError('legacy FF/Hebb/reward/eligibility/gate/phase knobs do not define the predictive energy')
            if (self.content_T,self.route_T,self.route_budget,self.hop_loss)!=(1.,1.,1.,'all'):
                raise ValueError('content_T, route_T, route_budget and hop_loss are legacy-emitter settings')
...
class RREM(PredictiveEnergy):
    ...
    def __init__(self,cfg: Cfg):
        ...
        self.rates={}  # signal optimizer buffers are allocated only when used
    ...
    def init_state(self,B: int):
        ...
        return State(z(B,D),z(B,D),z(B,D),z(B,len(self.cfg.trace_taus),D),
                     z(B,self.max_lag,D),z(B,D),z(B,self.cfg.M,D) if self.cfg.carry_elig else None)
    ...
    def tick(self,st: State,I: torch.Tensor,learn: bool=False,*,hops: int|None=None):
        ...
        return {'u':u,'ref':ref,'msgs':msgs,'routes':routes,'ch':ch,'records':records,
                'elig':elig.detach() if self.cfg.carry_elig else None}
    ...
    def advance(self,st: State,out: dict,valid: torch.Tensor,*,plastic: bool=False):
        ...
        if out.get('elig') is not None:
            st.elig=torch.where(v[:,None],out['elig'],st.elig)
    ...
    def _input_update(self,dI: torch.Tensor,byte: torch.Tensor):
        ...
        e=self.input_embedding()
        row=e[byte];raw_norm=row.norm(dim=-1,keepdim=True)
        norm=raw_norm.clamp_min(1e-8);unit=row/norm
        signal=dI[:,:self.cfg.N]
        dr=self.cfg.in_gain*(signal-unit*(signal*unit).sum(-1,keepdim=True)*(raw_norm>=1e-8))/norm
    ...
    def apply_batch(self):
        ...
        for name in self.param_names:
            ...
            if self.cfg.optimizer!='adam':
                if name not in self.rates:self.rates[name]=SignalRate(getattr(self,name),self.cfg.elig_decay)
    ...
    def from_checkpoint(cls,d: dict,device: str|None=None):
        ...
        for n,s in d['rates'].items():
            m.rates[n]=SignalRate(getattr(m,n),m.cfg.elig_decay)
            m.rates[n].load_state_dict({k:(v.to(m.dev) if isinstance(v,torch.Tensor) else v) for k,v in s.items()})
...
@torch.no_grad()
@cached_weights
def evaluate(mach: RREM,batches: Iterable,*,hops: int|None=None,per_document: bool=False):
    ...
    for b in batches:
        ...
        if per_document:
            doc_ce=torch.zeros(b.x.shape[0],cfg.L,cfg.H_pred,device=mach.dev,dtype=torch.float64)
            doc_cnt=torch.zeros(b.x.shape[0],cfg.H_pred,device=mach.dev,dtype=torch.int64)
        ...
        for t in range(b.T-1):
            ...
            if t>=b.P-1:
                ...
                for l in range(cfg.L):
                    ...
                    if per_document:doc_ce[:,l]+=(c*V).double()
                    correct[l]+=((lg.argmax(-1)==Y)&V).sum(0)
                if per_document:doc_cnt+=V
        ...
        if per_document:
            documents.extend({'id':int(i),'nll_sum':nll,'counts':count} for i,nll,count in
                             zip(b.doc_ids,doc_ce.cpu().tolist(),doc_cnt.cpu().tolist()))
    ...
    result={'bpb':bpb.cpu().tolist(),'read_level':rl,'bpb_h1':float(bpb[rl,0]),'bpb_mean_all_h':float(bpb[rl].mean()),
    ...
    if per_document:result['documents']=documents
    return result
