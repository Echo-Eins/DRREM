"""Content-controlled unitary shift pilot; final decoder, CE+7 MTP, ordinary Adam.
...
from drrem.core.phase_shift_transport import PhaseShiftTransportMachine,ShiftConfig,model_from_shift_protocol
...
def main():
    ...
    for arm in a.arms:
        ...
        m=PhaseShiftTransportMachine(cfg,MemoryConfig(arm,a.chunk),ShiftConfig()).to(device)
        copied=[]
        if parent:
            old=model_from_shift_protocol(reference) if 'content_shift' in reference else model_from_protocol(reference)
            missing,unexpected=m.load_state_dict(parent['model'],strict=False)
            if unexpected or any(not n.endswith(('read_scale','write_strength.weight','write_strength.bias','phase_shift.weight')) for n in missing):
        ...
        files=['scripts/train_phase_shift_transport.py','drrem/core/phase_shift_transport.py','drrem/core/nondecay_decode.py','drrem/core/nondecay_transport.py','drrem/core/causal_transport.py',
        ...
        protocol={'model':asdict(cfg),'data':reference['data'],'seed':reference['seed'],'batch':batch,
                  ...
                  'parameters':sum(v.numel() for v in m.parameters()),'test_opened':False,'content_shift':asdict(m.shift_config),
        ...
        while step<start_step+a.steps and not stopping:
            ...
            shift_grads=[float(layer.phase_shift.weight.grad.norm()) for layer in m.temporal] if edge_grad else []
            ...
            rec={'train_h1_bpb':float(s[0]/c[0])/math.log(2),'seconds':elapsed,'lr':lr,
                 ...
                 'peak_allocated_gib':torch.cuda.max_memory_allocated()/2**30,'phase_shift_gradient_norms':shift_grads}
            if (step-start_step)%a.eval_every==0 or step==start_step+a.steps:rec['dev']=assess();save()
            emit(rec)
        if not stopping:
            controls=[layer.phase_shift.weight.detach().clone() for layer in m.temporal]
            with torch.no_grad():
                for layer in m.temporal:layer.phase_shift.weight.zero_()
            disabled=evaluate(m,dev,device,protocol['precision'])
            with torch.no_grad():
                for layer,weight in zip(m.temporal,controls):layer.phase_shift.weight.copy_(weight)
            emit({'event':'phase_control_lesion','dev_with_content_shift_disabled':disabled,
                  'learned_control_weight_norms':[float(weight.norm()) for weight in controls]})
