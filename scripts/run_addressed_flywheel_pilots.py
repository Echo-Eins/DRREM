"""Causal correction addressing versus equal-capacity direct and pooled controls."""
import argparse
import json
...
def main():
    ...
    status=dict(stage='running',steps_per_arm=a.steps,started=time.time(),trials=[],test_opened=False,
        policy='same parent,data cursor,full Adam,mb1; response-only correction,adapter3e-5; no endpoint selection',
        claims='isolates context addressing; smaller step and prompt protection are shared by all arms')
    def save():
        path=a.out/'status.tmp';path.write_text(json.dumps(status,indent=2)+'\n');path.replace(a.out/'status.json')
    save()
    for name,mode in [('direct','off'),('addressed','addressed'),('uniform','uniform')]:
        command=[sys.executable,'-m','scripts.train_addressed_flywheel','--steps',str(a.steps),'--compile-parts',
            '--out',str(a.out/name),'--response-only','--adapter-lr','3e-5','--memory-mode',mode]
        ...
        ref=[json.loads(s) for s in (a.out/'direct/metrics.jsonl').read_text().splitlines()]
        reference=[r for r in ref if 'dev' in r][-1]['dev']
        item['versus_direct']=paired_difference(row['dev']['documents'],reference['documents'])
