"""Matched Adam experiments with a hard one-pass response-byte budget.
...
import drrem.core.adaptive_phase_transport as phase_module
...
def main():
    ...
    p.add_argument('--variant',choices=['attention','query_phase','query_anchor',*VARIANTS],required=True)
    ...
    if a.variant=='attention':m=CausalTransportMachine(cfg)
    elif a.variant in ('query_phase','query_anchor'):
        m=QueryPhaseTransportMachine(cfg,replace(VARIANTS['ring_frequency'],chunk=a.chunk),
                                    QueryPhaseConfig(anchor=a.variant=='query_anchor'))
    else:m=AdaptivePhaseTransportMachine(cfg,replace(VARIANTS[a.variant],chunk=a.chunk))
    m=m.to(device)
    ...
    files=['scripts/train_adaptive_phase.py','drrem/core/adaptive_phase_transport.py','drrem/core/query_phase_transport.py',
    ...
    protocol={'model':asdict(cfg),'data':reference['data'],'seed':reference['seed'],'batch':batch,
              ...
              'execution':{'compile_scans':a.compile_scans,'torch_compile':a.compile_model,'checkpoint_hops':False,
    ...
    if a.variant!='attention':protocol['adaptive_phase']=asdict(m.phase_config)
    if a.variant in ('query_phase','query_anchor'):protocol['query_phase']=asdict(m.query_config)
    ...
    if a.parent:
        ...
        for key in ['model','data','seed','batch','precision','mtp_weight','optimizer','lr_schedule','adaptive_phase','query_phase']:
    ...
    if a.compile_scans:
        # Compile only the shared chunk kernels, not 18 copies of the full graph.
        phase_module.phase_scan=torch.compile(phase_module.phase_scan,dynamic=False)
        phase_module.ridge_scan=torch.compile(phase_module.ridge_scan,dynamic=False)
    ...
    while step<last_step and not stopping:
        ...
        with autocast(device,protocol['precision']):
            loss,s,c=response_objective(forward(b.x[:,:-1],b.active[:,:-1]),b.x,b.loss_mask[:,:-1],b.active[:,:-1],a.mtp_weight)
        ...
        phase_grad={name:float(param.grad.norm()) for name,param in m.named_parameters()
                    if diagnostic and any(key in name for key in ['frequency_offset','query_phase','query_rotation','anchor_logit','log_ridge','read_scale','write_strength'])}
