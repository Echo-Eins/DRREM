"""Matched no-decay temporal-memory pilots; final decoder, CE+7 MTP, ordinary Adam.
...
def main():
    ...
    p.add_argument('--arms',nargs='+',choices=['attention','phase_sum','phase_delta','byte_bank'],required=True)
    ...
    for arm in a.arms:
        ...
        m=(CausalTransportMachine(cfg) if arm=='attention' else NondecayTransportMachine(cfg,MemoryConfig(arm,a.chunk))).to(device)
        copied=[]
        if parent:
            old=model_from_protocol(reference)
        ...
        files=['scripts/train_nondecay_transport.py','drrem/core/nondecay_transport.py','drrem/core/causal_transport.py',
